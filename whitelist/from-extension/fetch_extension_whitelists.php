 ...
 *
 * Pulls the user-whitelisted-domains CSV from each extension backend and stores
 * it in this folder as extension-<id>.csv. Run by the GitHub Actions workflow
 * .github/workflows/fetch-extension-whitelists.yml (daily + manual), which then
 * commits whatever changed — git history is the archive; the endpoints
 * themselves are stateless and store nothing.
 *
...
if (!defined('WLF_SKIP_MAIN')) {
    ...
    $failures = wlf_run($EXTENSIONS, __DIR__);
